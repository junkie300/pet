...
1. **지도(S-02) ③ 카테고리 필터 칩** ← **여기서 시작한다.**
   ① 지도+핀 과 ② 바텀시트 3단은 **실기기 확인까지 끝났다** (D-70·D-73·D-74·D-75).
   · 칩은 지도 상단에 복수 선택으로 놓는다 (`spec.md §5.2`). 지금 적재된 것은 동물병원뿐이라
     **나머지 칩은 "준비 중"으로 눌리지 않게** 둔다 (홈 그리드가 `PlaceCategory.loaded` 로 하는 것과 같다)
   · 이때 **홈 타일도 목록이 아니라 지도로 돌린다.** 탭 이동은 반드시 `switchTab` 이다 (D-55)
   · ⚠️ 칩이 붙으면 조회가 카테고리 하나가 아니게 된다. `PlaceListViewModel` 은 카테고리 하나를
     받는 구조이므로, **여기서 지도용으로 갈라질지 함께 갈지**를 먼저 정한다 (D-70 의 재사용 결정)
   · 그 다음이 ④ 클러스터링이다. **핀 이름이 겹치는 것은 거기서 푼다** — 여백으로 풀면 축척만 물러난다 (D-73)
...
5. **앱 아이콘** — 서체까지 왔으니 남은 임시값은 아이콘 하나다
