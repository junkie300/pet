...
1. **지도(S-02) ①을 실기기에서 눈으로 확인** ← **여기서 시작한다.**
   SDK 연동과 핀까지 코드는 붙었다 (D-70). 그런데 **카카오맵 SDK 는 ARM 전용**이라
   이 PC 의 x86_64 에뮬레이터에서는 지도를 열 수 없다 — 앱은 정상이고 지도 탭만 안내가 뜬다 (D-71).
   · 폰을 USB 로 연결하고(개발자 옵션 > USB 디버깅) `cd D:\pet\app && gradlew installDebug`
   · 지도 탭에서 **고른 읍면동의 동물병원이 제자리에 뜨는지**, 핀을 누르면 상세로 가는지 본다
   · ⚠️ 지도만 **까맣게** 뜨면 코드가 아니라 콘솔의 패키지명·키 해시를 먼저 본다 (D-69).
     키 해시는 **빌드한 PC 의 debug 인증서** 것이어야 한다
2. **지도(S-02) 나머지** — ①이 화면으로 확인된 뒤에 이어서 한다
   · ② 바텀시트 3단 → ③ 카테고리 필터 칩 → ④ 클러스터링 순서다
   · 목록·상세·카드(`ui/place/`)는 이미 있다. **바텀시트 안에 그대로 들어간다**
   · 지도는 목록과 **같은 `PlaceListViewModel`** 을 본다 (D-70). 바텀시트도 그대로 이어받는다
   · 홈 타일은 아직 목록으로 간다. **필터 칩이 붙는 ③에서** 지도로 돌린다 (탭 이동은 `switchTab` — D-55)
