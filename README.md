...
- [◐] **지도(S-02)** — 네이티브 앱 키 발급 + 콘솔 등록 (2026-09-06) 뒤로 ①②③ 을 붙였다
      ...
      → ⚠️ **④ 는 아직 실기기에서 못 봤다.** 핀이 많은 동네(강남구 역삼동 등)로 확인해야 한다
...
#### B. 그 다음 코드 (에뮬레이터로 확인된다)

1. **지도 "현재 위치로" 버튼** (`spec.md §5.2`) — 위치 권한은 D-81 로 이미 붙었다.
   버튼과 카메라 이동만 더하면 된다. 단, 확인은 실기기 몫이다
2. **"이 지역에서 다시 검색"** (`spec.md §5.2`) — 선택 지역 밖으로 지도를 옮기면 떠야 하는데
   지도 ①~④ 어디에도 들어 있지 않아 빠졌다 (D-78)
