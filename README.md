...
- [ ] **장소 목록** ← 홈에서 `동물병원 8곳` 을 눌러도 그 8곳을 볼 화면이 없다. **다음은 여기다**
...
1. **장소 목록 화면** ← **여기서 시작하는 것을 권한다**
   - 홈에서 `동물병원 8곳` 을 눌렀을 때 **그 8곳을 보여줄 화면이 아직 없다.** 지금은 지도 안내 화면으로 간다
   - 지도 없이 목록으로 먼저 만든다. 카카오 키를 기다리지 않아도 된다
   - `PlaceRepository` 에 `listByRegion(regionCode, category)` 를 더하면 된다.
     건수 질의가 이미 같은 필터를 쓰고 있다
   - 카드 규격은 `spec.md §6.4` — 이름 · 카테고리 배지 · 주소 1줄 · 영업상태
2. **S-03 장소 상세** — 이름·주소·전화(탭하면 발신)·영업상태·**출처와 기준일**.
   길찾기만 지도 키가 필요하고 나머지는 지금 다 된다. **별점·후기·사진은 넣지 않는다** (D-40)
3. **S-07 즐겨찾기** (Room) — 로그인 불필요
4. **오프라인 캐시** — 아래 '알려진 빈틈' 참고
5. **Pretendard 번들** — 화면이 확정됐으니 지금이 적기다 (`Type.kt` 의 TODO)
