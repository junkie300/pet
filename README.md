...
1. **2단계 미용시설** ← **여기부터. C-1 만 누르면 막힌 곳이 없다** (D-93)
   여기서 `plan.md` 의 진짜 시험이 시작된다 — **앱 코드 수정이 "필터 칩 1줄" 수준이어야 한다.**
   화면을 새로 만들어야 한다면 1단계 설계가 틀린 것이므로 **되돌아가 고친다.**
   · 원천은 **공공데이터포털 `15154944`**(행정안전부_동물_동물미용업) —
     **동물병원(15154952)의 형제 서비스**다. 응답 필드·EPSG:5174·영업상태코드가 같다
   · 엔드포인트는 `https://apis.data.go.kr/1741000/pet_grooming/info` 로 **보인다**.
     문서에 안 나와 게이트웨이에 물어 좁힌 것이다 — **없는 이름은 400, 있는데 미신청이면 403**
     이고 이 이름이 403 이다. 신청하면 마이페이지가 최종 주소를 확정해 준다
...
> ⚠️ **클러스터링(④)은 2단계 전까지 실데이터로 문턱에 닿지 않는다.** 영업중 기준 한 읍면동
> 최대가 30곳(송도동)인데 문턱은 50이다 → 볼 일이 있으면 `MapClustering.THRESHOLD` 를 잠깐
> **5로 낮춘 debug 빌드**로 본다. 확인이 끝나면 반드시 50으로 되돌린다 (D-83).

#### C. 승인·발급 대기 (개발과 병행)

1. **공공데이터포털 `15154944` 활용신청** ← **2단계를 여는 유일한 열쇠**
   https://www.data.go.kr/data/15154944/openapi.do · **자동승인**(개발계정 10,000건/일)이라
   대기랄 것이 없다. **키는 새로 안 받는다** — 계정당 1개인 `DATA_GO_KR_KEY` 가 그대로 열린다
...
- **클러스터링을 실데이터로 볼 수 없다** (D-83). 문턱 50인데 영업중 기준 한 읍면동 최대가
  30곳(송도동)이다. 2단계 미용시설이 들어오면 강남·서초에서 실제로 넘게 된다
