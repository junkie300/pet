...
```
cd D:\pet\app
gradlew testDebugUnitTest     # 71개 통과해야 함 (기기·네트워크 불필요)
...
**앱에서 `spec.md §5.2` 가 요구한 화면이 다 찼다 (2026-09-09).** 지도(S-02)에 이어
S-01 의 마지막 한 줄인 **「현재 위치로」**(D-91)까지 닫았다. 무엇을 왜 그렇게 정했는지는
`DECISIONS.md` §49~§54.

**1단계에 앱으로 남은 것은 없다.** 남은 칸은 ETL 쪽이다 — 동물병원 데이터는 이미 들어와
있고(10,617건), `plan.md` 1단계의 활용신청·좌표계 검증 항목이 문서로 남아 있다.

⚠️ **확인 대기가 하나 생겼다: 「현재 위치로」를 진짜 위치로 한 번 봐야 한다.** 에뮬레이터는
위치를 못 잡는다 — `geo fix` 가 OK 를 돌려줘도 GNSS 가 한 점도 주지 않고, 셸은 mock 도 못
놓는다 (D-91). 그래서 성공 경로만 **잠깐 고친 debug 빌드**로 봤다. 나머지 경로(안 묻는다 ·
거절 · 못 잡음)는 에뮬레이터에서 확인 끝.
...
2. **좌표가 다른데도 이름끼리 겹친다** (D-86 실측) — 급하지 않지만 실기기에서 눈에 띈다.
   자양동 `로얄동물병원 · 자양동물병원 · 해맑은동물병원` 이 겹쳐 찍힌다. D-85 가 고친 것은
   좌표가 **같은** 경우이고, 이건 겹침 경쟁(`CompetitionType.All`)이 갈라 줘야 하는데 듣지 않는다
   · 의심되는 곳: 이름 레이어의 아이콘이 `spacerBitmap` 이라 **경쟁 사각형이 글자를 못 덮는 것**
     (`MapScreen` 의 `nameStyles` · `layerFor(NAME_LAYER_ID, CompetitionType.All, ...)`)
   · ⚠️ 확인은 실기기 몫이다 (D-71). 단위 테스트로는 안 잡힌다 — 경쟁은 SDK 안에서 일어난다

#### B. 다음에 폰을 꽂았을 때 볼 것

**밀린 것은 하나다** — 「현재 위치로」를 **진짜 위치로** 보는 것이다 (D-91).
나머지는 2026-09-09 에 D-85·D-86·D-87·D-88·D-90 을 실기기(SM-S711N)에서,
D-89·D-91 을 에뮬레이터에서 봤다.
...
- [ ] **A-2 를 고친 뒤** 자양동으로 들어가 `로얄 · 자양 · 해맑은동물병원` 세 이름이 갈라지는지
...
- **좌표가 *다른데도* 이름끼리 겹쳐 찍힌다** (D-86 실측). 자양동의 `로얄동물병원 · 자양동물병원 ·
  해맑은동물병원` 이 그렇다. D-85 가 고친 것은 **좌표가 같은** 경우이고, 이건 겹침 경쟁
  (`CompetitionType.All`)이 갈라 줘야 하는데 듣지 않는다 — 이름 레이어의 아이콘이 spacer 라
  경쟁 사각형이 글자를 못 덮는 것으로 의심된다 (미확인). D-73 계열의 남은 문제다
