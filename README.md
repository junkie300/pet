...
- [◐] **지도(S-02)** — 네이티브 앱 키 발급 + 콘솔 등록 (2026-09-06) 뒤로 ①②③ 을 붙였다
      ...
      → **실기기(SM-S711N) 확인 완료** (2026-09-09 · D-85·D-86)
...
1. **다크 모드에서 지도만 밝다** (D-74) — 시트가 검은데 지도가 흰 것은 눈에 띈다.
   `MapViewInfo` 의 지도 스타일 문제이며, 지도를 열 수 있어야 확인된다 (D-71)
2. **즐겨찾기 카드의 거리** (D-81) — 좌표는 스냅샷에 있다. 다만 즐겨찾기는 **전국이 섞이므로**
   거리가 무엇을 뜻하는지 먼저 정한다 (목록은 "고른 지역 안에서 어느 쪽이 가까운가"였다)
3. **지도 탭의 `목록으로 보기`** — 지도를 못 여는 기기(D-71)에서 지도 탭은 안내 한 장이다.
