...
1. **Pretendard 번들** ← **여기서 시작하는 것을 권한다.** 화면이 확정됐으니 지금이 적기다
   (`Type.kt` 의 TODO). 서체는 화면 전체의 인상을 바꾸므로 지도(S-02)가 붙기 전에 하는 게 낫다
2. **캐시를 먼저 그리고 뒤에서 갱신 (cache-first)** — 지금은 사본이 뜨기까지 8~16초 걸린다
   (아래 '알려진 빈틈'). 저장소가 값을 두 번 흘려보내야 해서 `suspend` → `Flow` 전환이 따라온다
