...
S-01 의 마지막 한 줄인 **「현재 위치로」**(D-91)까지 닫았다. 이름 겹침(D-92)도 함께 고쳤다.
무엇을 왜 그렇게 정했는지는 `DECISIONS.md` §49~§55.

**1단계에 앱으로 남은 것은 없다.** 남은 칸은 ETL 쪽이다 — 동물병원 데이터는 이미 들어와
있고(10,617건), `plan.md` 1단계의 활용신청·좌표계 검증 항목이 문서로 남아 있다.

⚠️ **확인 대기가 둘 생겼다. 둘 다 실기기에서만 볼 수 있다.**
- **「현재 위치로」를 진짜 위치로** (D-91) — 에뮬레이터는 위치를 못 잡는다. `geo fix` 가 OK 를
  돌려줘도 GNSS 가 한 점도 주지 않고, 셸은 mock 도 못 놓는다. 성공 경로만 **잠깐 고친 debug
  빌드**로 봤다. 나머지(안 묻는다 · 거절 · 못 잡음)는 에뮬레이터에서 확인 끝
- **이름 겹침이 풀렸는지** (D-92) — 지도는 ARM 실기기가 있어야 열린다 (D-71)
