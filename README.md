...
- [ ] 지도(S-02) ← 카카오 **네이티브 앱 키** 대기. **여기만 남았다.**
      목록은 이미 있으니 바텀시트에 넣으면 된다
...
1. **오프라인 캐시** ← **여기서 시작하는 것을 권한다**
   - 즐겨찾기가 이미 오프라인으로 뜨므로, 남은 것은 **최근 조회 결과**와 **상세**다
   - 즐겨찾기 스냅샷에 `source`·`source_updated_at` 을 더하면 오프라인 상세도 열린다 (D-61 한계)
   - S-01 의 최근 지역 칩도 여기서 함께 고친다 (아래 '알려진 빈틈')
2. **Pretendard 번들** — 화면이 확정됐으니 지금이 적기다 (`Type.kt` 의 TODO)
...
- **오프라인에서 즐겨찾기를 눌러 상세를 열면 실패 화면이 뜬다.** 목록은 스냅샷으로 뜨지만
  상세는 서버를 다시 부른다. 스냅샷에 출처·기준일이 없어 그대로 그리면 의무 표기를 어긴다 (D-61).
  오프라인 캐시와 함께 푼다
