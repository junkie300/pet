...
**`spec.md §5.2` 의 지도 명세가 다 찼다 (2026-09-09).** 「이 지역에서 다시 검색」과 같은 좌표
이름 겹침이 마지막이었다 — 무엇을 왜 그렇게 정했는지는 `DECISIONS.md` §49.
홈 → 목록 → 상세 → 즐겨찾기가 실제 데이터로 이어지고, 網이 끊겨도 사본이 **먼저** 뜬다.

**실기기 확인까지 끝났다** (2026-09-09 · SM-S711N). 무엇을 어떻게 봤는지는 `DECISIONS.md` §49.

#### A. 지금 이어서 할 코드 ← **여기서 시작한다**

1. **2단계 미용시설** — LOCALDATA 인증키가 오면 바로 시작한다 (아래 C). 여기서
...
- [x] ~~「이 지역에서 다시 검색」(D-86) · 같은 좌표 이름 겹침(D-85)~~ — 2026-09-09 확인 완료
- [x] ~~테마 아이콘(Android 13+)~~ — 2026-09-09 확인 완료. 뱃지가 얼룩으로 남던 것을 고쳤다 (D-90)
