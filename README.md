...
- [◐] **지도(S-02)** — 네이티브 앱 키 발급 + 콘솔 등록 (2026-09-06) 뒤로 ①②③ 을 붙였다
      ...
      → 남은 것은 **④ 클러스터링** 하나다. **1단계에 남은 것도 이것 하나다**
...
1. **지도(S-02) ④ 클러스터링** ← **여기서 시작한다. 1단계의 마지막 조각이다.**
   ①~③(지도+핀 · 바텀시트 3단 · 필터 칩)은 **실기기 확인까지 끝났다** (D-70·D-73~D-76).
   · `spec.md §5.2` 는 **핀 50개 초과 시 클러스터링**을 요구한다. 삼성동은 8곳이라 안 걸린다 —
     **핀이 많은 동네**(강남구 역삼동 등)로 확인해야 한다
   · ⚠️ **핀 이름이 서로 겹치고 화면 끝에서 잘리는 것도 여기서 함께 푼다.** 여백을 키워
     풀려고 하면 축척만 물러난다 (D-73 실측)
   · 카카오맵 SDK 는 `LodLabelLayer`(LOD 라벨)를 갖고 있다. 직접 묶는 것보다 먼저 그걸 본다
2. ⚠️ **지도를 고쳤으면 실기기에서 본다.** 카카오맵 SDK 는 **ARM 전용**이라 이 PC 의 x86_64
...
3. **캐시를 먼저 그리고 뒤에서 갱신 (cache-first)**
...
5. **아이콘을 런처에서 눈으로 본다** — 에셋과 배선은 끝났다 (D-77). 남은 것은 확인뿐이다.
   `gradlew installDebug` 뒤에 **원형 런처 · 폴더 안 · 설정 > 앱 목록 · 테마 아이콘(Android 13+)**
   네 곳을 본다. 시안을 바꾸고 싶으면 `python tools/make_launcher_icon.py` 로 다시 뽑는다

#### 승인·발급 대기 (개발과 병행)

6. **LOCALDATA 가입 + 인증키** — https://www.localdata.go.kr (공공데이터포털 키는 안 통한다).
   **2단계 미용시설**에 필요하다. `localdata_cd` 는 문서로 이미 끝냈으므로 급하지 않다
7. **테스터 12명 명단** — 리드타임이 가장 길다 (`plan.md` §3). 개발과 무관하게 지금부터 모은다
8. **KIPRIS 상표 조회** — 「미리펫」을 제9류(소프트웨어)·제42류(SaaS)로 확인 (D-43).
   스토어 등록 전까지만 하면 된다
