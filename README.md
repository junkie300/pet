...
- [◐] **지도(S-02)** — 네이티브 앱 키 발급 + 콘솔 등록 (2026-09-06) 뒤로 ①②③ 을 붙였다
      ...
      → ⚠️ **위 둘은 실기기 확인이 남았다** (D-71). 코드와 단위 테스트(12개)까지만 끝났다
...
⚠️ **방금 붙인 둘(D-85·D-86)은 실기기 확인이 남았다.** 지도는 ARM 기기에서만 열린다 (D-71).
아래 B 가 그 목록이다.
...
- [ ] **「이 지역에서 다시 검색」** (D-86) — 지도를 3km 넘게 밀면 상단에 버튼이 뜬다.
      · 누르면 **가장 가까운 읍면동으로 갈아타고** `○○ 기준으로 바꿨습니다` 가 3.5초 뜬다
      · 갈아탄 뒤 **버튼이 저절로 사라지는지** (새 지역 중심에 카메라가 오므로 거리가 0에 가깝다)
      · 홈 탭으로 가면 **홈의 지역도 함께 바뀌어 있어야 한다** (D-54 — 같은 현재 지역이다)
      · 網을 끊고 눌러 보면 `이 근처에서 지역을 찾지 못했습니다` 이거나, 받아 둔 지역 중 하나다
- [ ] **같은 좌표 이름 겹침** (D-85) — 역삼동으로 들어가 `온숲` 두 곳이
      `24시 온숲 동물의료센터 외 1` **하나로만** 뜨는지. 핀은 그대로 있어야 한다
