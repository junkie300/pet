...
```
cd D:\pet\app
gradlew testDebugUnitTest     # 55개 통과해야 함 (기기·네트워크 불필요)
...
**실기기 확인이 끝났다 (2026-09-08 · SM-S711N). 이제 남은 것은 코드다.**
홈 → 목록 → 상세 → 즐겨찾기가 실제 데이터로 이어지고, 網이 끊겨도 사본이 **먼저** 뜬다.
지도는 ①~④ 와 `현재 위치로` 까지 눈으로 봤다 — 무엇이 어떻게 나왔는지는 `DECISIONS.md` §48.

#### A. 지금 이어서 할 코드 ← **여기서 시작한다**

1. **「이 지역에서 다시 검색」** (`spec.md §5.2`) — **지도 명세 중 마지막 남은 하나다.**
   선택 지역 밖으로 지도를 옮기면 떠야 하는데 ①~④ 어디에도 안 들어갔다 (D-78)
   · 붙일 자리는 `MapScreen` 의 상단 알림 줄이다. 카메라가 멈출 때를 받는 콜백은 **이미 있다** —
     `setOnCameraMoveEndListener`(배율을 받아 다시 묶는 그 자리)에 중심 좌표를 같이 받으면 된다
   · ⚠️ **경계를 모른다.** `regions` 에는 읍면동 **중심좌표**만 있고 다각형이 없다 → "고른 지역
     중심에서 얼마나 멀어지면 뜨는가"로 정할지 먼저 결정한다. 반경을 쓰면 그 값이 곧 규칙이 된다
   · 누르면 그 자리 기준으로 다시 조회해야 하는데, 저장소는 **읍면동 코드로** 조회한다
     (`listByRegion`) — 좌표로 조회하는 길이 없다. **여기서 설계가 한 겹 늘어난다**
   · 확인은 실기기 몫이다 (D-71)
2. **같은 좌표 장소의 이름 겹침** (D-83) — 역삼동 `온숲` 두 곳처럼 좌표가 같으면 이름이 겹쳐
   찍힌다. 겹침 경쟁이 그걸 못 지운다 → **같은 자리 묶음은 이름을 하나만 그린다.**
   고칠 자리는 `MapScreen` 의 `nameOptions` 를 만드는 곳 한 군데다
3. **다크 모드에서 지도만 밝다** (D-74) — `MapViewInfo` 의 지도 스타일 문제다
4. **즐겨찾기 카드의 거리** (D-81) — 좌표는 스냅샷에 있다. 다만 즐겨찾기는 **전국이 섞이므로**
   거리가 무엇을 뜻하는지 먼저 정한다
5. **2단계 미용시설** — LOCALDATA 인증키가 오면 바로 시작한다 (아래 C). 여기서
...
- [ ] **테마 아이콘**(Android 13+) — 런처 설정에서 테마 아이콘을 켜고 본다. 단색 레이어 자체는
      ...
      2026-09-08 에 확인했다
- [ ] **위 A-2 를 고친 뒤** 역삼동으로 들어가 `온숲` 두 곳의 이름이 하나만 뜨는지
- [ ] **위 A-1 을 붙인 뒤** 지도를 고른 지역 밖으로 밀어 본다
...
- **같은 좌표에 있는 두 장소의 이름이 겹쳐 찍힌다** (D-83). 역삼동 `24시 온숲 동물의료센터` 와
  `온숲 영상·외과 동물의료센터` 는 좌표가 글자까지 같은데 이름 레이어의 겹침 경쟁이 하나를
  지우지 않는다. 핀은 멀쩡하고 이름만 읽을 수 없다 — 같은 자리 묶음은 이름을 하나만 그려야 한다
