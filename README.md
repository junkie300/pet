...
#### A. 실기기(SM-S711N)에서 볼 것 — **2026-09-08 에 대부분 끝냈다**

폰을 USB 로 연결하고(개발자 옵션 > USB 디버깅) `cd D:\pet\app && gradlew installDebug`.

- [x] **지도 ④ 클러스터링** (D-78) — **`THRESHOLD` 를 잠깐 5로 낮춰서** 봤다. 1단계 데이터로는
      문턱 50에 닿는 읍면동이 없다(영업중 최대 송도동 30곳 · D-83)
      · 묶음 개수 합 = 시트 건수(8+5+2=15) · 개수가 원 한가운데 · 원끼리 안 겹침 — 전부 맞다
      · **누르면 갈라지는 것은 고친 뒤에야 됐다** (D-83)
- [x] **거리 표기** (D-81) — 카드에 `25km`. 값이 실제 거리와 맞다
- [x] **지도의 `현재 위치로`** (D-82) — 버튼·카메라·파란 점·두 안내(거절/못 잡음) 전부 확인
- [x] **지도 시트 안의 목록** (D-79) — 網을 끊고 들어가 **사본이 먼저** 그려지고 배너가 뒤에 붙었다
- [x] **앱 아이콘** (D-77) — 앱 서랍 · 설정 > 애플리케이션 정보(원형 마스크). 잘림 없음
- [ ] **테마 아이콘**(Android 13+) — 런처 설정에서 테마 아이콘을 켜고 봐야 한다. 단색 레이어
      자체는 실루엣으로 제대로 뽑혀 있다
- [ ] **같은 좌표 장소의 이름 겹침** — 역삼동 `온숲` 두 곳에서 이름이 겹쳐 찍힌다 (아래 빈틈)
