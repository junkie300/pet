...
```
cd D:\pet\app
gradlew testDebugUnitTest     # 67개 통과해야 함 (기기·네트워크 불필요)
...
**지도(S-02)가 다 찼다 (2026-09-09).** `spec.md §5.2` 의 지도 명세(D-85·D-86)에 더해
다크 모드 지도(D-87) · 즐겨찾기 거리(D-88) · 지도를 못 여는 기기의 빠져나갈 길(D-89) ·
테마 아이콘(D-90)까지 닫았다. 무엇을 왜 그렇게 정했는지는 `DECISIONS.md` §49~§53.

**확인 대기 중인 것은 없다** — 실기기(SM-S711N)와 에뮬레이터 양쪽 다 봤다.

⚠️ **1단계에 하나가 남아 있다** — `spec.md §5.2` S-01 의 **「현재 위치로」 버튼**이다.
지도의 그것(D-82)과 **다른 화면**이라 따로 남았다. 2단계로 넘어가기 전에 이걸 닫는 게 맞다.

#### A. 지금 이어서 할 코드 ← **여기서 시작한다**

1. **S-01 의 「현재 위치로」** (`spec.md §5.2`) ← **여기부터** — 1단계에 남은 마지막 하나다.
   지역 선택 화면에서 누르면 **현재 위치의 읍면동으로** 곧장 간다. 지도의 `현재 위치로`(D-82)는
   카메라만 옮기는 것이라 다른 일이다
   · **필요한 조각이 이미 다 있다** — `RegionRepository.nearestDong(GeoPoint)`(D-86) ·
     `DeviceLocation`(D-81) · `rememberLocationPermissionRequest`. 새로 만들 것은 화면 쪽 버튼뿐이다
   · 규칙도 이미 정해져 있다: **먼저 묻지 않는다**(D-81) · 못 갔으면 **이유를 한 줄로**(D-82) ·
     고른 결과는 `RecentRegionStore.remember` 로 넣는다(D-86 과 같은 길)
   · ⚠️ 명세가 못박은 것 — **권한을 거절해도 지역 선택은 그대로 되어야 한다.** 버튼 하나가
     안 될 뿐이지 화면이 막히면 안 된다
   · 붙일 자리: `ui/region/RegionPickerScreen.kt` (검색창과 최근 지역 칩 사이)
2. **2단계 미용시설** ← **키가 오면 여기부터** — LOCALDATA 인증키가 필요하다 (아래 C).
...
3. **좌표가 다른데도 이름끼리 겹친다** (D-86 실측) — 급하지 않지만 실기기에서 눈에 띈다.
...
**밀린 것은 없다.** 2026-09-09 에 D-85·D-86·D-87·D-88·D-90 을 실기기(SM-S711N)에서,
D-89 를 에뮬레이터에서 다 봤다. 아래는 위 A 를 손댄 **뒤에** 볼 것들이다.

`cd D:\pet\app && gradlew installDebug` (개발자 옵션 > USB 디버깅).

- [ ] **A-1 을 붙인 뒤** S-01 에서 `현재 위치로` — 위치를 준 경우 · 거절한 경우 · 못 잡은 경우
- [ ] **A-3 을 고친 뒤** 자양동으로 들어가 `로얄 · 자양 · 해맑은동물병원` 세 이름이 갈라지는지
...
- **`spec.md §5.2` S-01 의 「현재 위치로」 버튼이 없다.** `RegionPickerScreen.kt` 에
  "카카오맵 SDK·위치 권한이 붙는 시점에 추가한다"는 메모만 있는데 **그 시점은 이미 지났다** —
  둘 다 붙어 있고 `nearestDong`(D-86)까지 생겼다. 위 A-1 이 이것이다. 급하지 않은 것이 아니라
  **1단계에 남은 유일한 명세 항목**이라 맨 위에 두었다
