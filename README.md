...
**1단계 화면은 다 붙었다. 남은 것은 지도 ④ 와 아이콘을 실기기에서 보는 일이다.**
홈 → 목록 → 상세 → 즐겨찾기가 전부 실제 데이터로 이어지고, 網이 끊겨도 사본으로 뜬다.

#### 지금 바로 할 수 있는 것 (대기 없음)

1. **지도(S-02) ④ 를 실기기에서 본다** ← **여기서 시작한다. 1단계의 마지막 조각이다.**
   코드와 단위 테스트는 끝났다 (D-78). ①~③ 은 실기기 확인까지 끝났다 (D-70·D-73~D-76).
   · **핀이 많은 동네로 들어간다** — 삼성동은 8곳이라 문턱(50)에 닿지도 않는다.
     강남구 역삼동처럼 빽빽한 곳이라야 묶음 원이 뜬다
   · 볼 것: **묶음 원의 개수 합 = 시트 건수** · 원을 누르면 갈라지는가 · 확대할수록 잘게
     나뉘는가 · **이름이 서로 겹치지 않는가** · 개수가 원 한가운데 찍히는가
   · 마지막 둘은 SDK 의 글자 배치에 기대는 부분이라 **화면으로만** 확인된다
   · ⚠️ `spec.md §5.2` 의 **"이 지역에서 다시 검색"** 버튼은 아직 없다 (D-78 아래)
