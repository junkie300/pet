...
- [ ] **S-00 홈 + 하단 탭 4개** (D-39) ← **막힌 것 없음. 바로 할 수 있다**
- [ ] S-03 상세 ← 길찾기만 지도 키가 필요하고 나머지는 지금 된다
- [ ] S-07 즐겨찾기 (Room) ← 막힌 것 없음
- [ ] 지도(S-02) ← 카카오 **네이티브 앱 키** 대기
...
1. **S-00 홈 + 하단 탭 4개** ← **여기서 시작하는 것을 권한다**
   - `places` 가 찼으므로 카테고리별 건수가 **실제 숫자**로 나온다. 껍데기가 아니다
   - 상단 지역 칩은 **반드시 읍면동까지** 표시한다 (D-39 — 이걸 시군구로 낮추면 제품이 흐려진다)
   - "내 주변 N km" 를 홈의 주인공으로 두지 않는다 (D-24·D-39)
   - 탭은 홈 · 지도 탐색 · 구조·입양 · 더보기. `spec.md §5.1` 참고
2. **S-03 장소 상세** — 이름·주소·전화(탭하면 발신)·영업상태·**출처와 기준일**.
   길찾기만 지도 키가 필요하고 나머지는 지금 다 된다. **별점·후기·사진은 넣지 않는다** (D-40)
3. **장소 목록 화면** — 지도 없이 목록으로 먼저. 읍면동 선택 → 그 동네 병원 목록
4. **S-07 즐겨찾기** (Room) — 로그인 불필요
5. **오프라인 캐시** — 아래 '알려진 빈틈' 참고
...
- **오프라인으로 앱을 시작하면 네트워크가 복구돼도 최근 지역 칩이 안 돌아온다.**
  `observeRecent` 가 실패 시 빈 목록을 넣고 다시 시도하지 않는다. 앱을 다시 켜면 된다.
...
- **Pretendard 폰트가 아직 번들되지 않았다** (`Type.kt` 의 TODO). 화면이 확정됐으니 지금이 적기다
