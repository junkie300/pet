...
- [x] **앱 아이콘** — 시안(핀 + 발바닥 + 방향 뱃지)을 브랜드 딥그린으로 옮겨 적응형 아이콘으로 (D-77)
      ...
      → **실기기 확인 완료** (2026-09-08) — 앱 서랍(스퀘어클)·설정 > 애플리케이션 정보(**원형**)
        둘 다 잘리지 않는다. 테마 아이콘(Android 13+)만 런처 설정을 켜고 보는 것이 남았다
...
1. **즐겨찾기 카드의 거리** (D-81) — 좌표는 스냅샷에 있다. 다만 즐겨찾기는 **전국이 섞이므로**
   거리가 무엇을 뜻하는지 먼저 정한다 (목록은 "고른 지역 안에서 어느 쪽이 가까운가"였다)
2. **지도 탭의 `목록으로 보기`** — 지도를 못 여는 기기(D-71)에서 지도 탭은 안내 한 장이다.
   홈 타일은 목록으로 돌려 놓았지만(D-80) 탭 자체에는 빠져나갈 길이 없다.
   `ComingSoonScreen` 에 동작 버튼 자리를 만드는 일이다
3. **2단계 미용시설** — LOCALDATA 인증키가 오면 바로 시작한다 (아래 C). 여기서
...
- [ ] **테마 아이콘**(Android 13+) — 런처 설정에서 테마 아이콘을 켜고 본다. 단색 레이어 자체는
      실루엣으로 제대로 뽑혀 있다(`mipmap/ic_launcher_monochrome`). 나머지 자리(앱 서랍·설정)는
      2026-09-08 에 확인했다
