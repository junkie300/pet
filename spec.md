...
|---|---|---|---|
| `hospital` 동물병원 | 공공데이터포털 15154952 (행안부 인허가) | 월~토 매일 19시 (D-2) | 매일 1회 |
| `grooming` 미용시설 | 공공데이터포털 15154944 (행안부 인허가) | 동일 | 매일 1회 |
...
- **지방행정 인허가 데이터**: 폐업·휴업 업소가 그대로 포함됨 → 상세영업상태코드 필터 **필수**.
  누락 시 폐업한 병원이 지도에 표시됨.
  ⚠️ **배포처가 바뀌었다** — `localdata.go.kr` 은 **2026-04-16 에 닫혔고** 인허가 데이터는
  공공데이터포털로 통합됐다 (D-93). 데이터 자체와 출처 표기(`지방행정인허가데이터`)는 그대로다.
...
- ~~LOCALDATA API 는 변경분만 준다~~ — **더는 해당하지 않는다** (D-93). 그 사이트가 닫혔고,
  공공데이터포털의 인허가 서비스는 **전체분을 쪽수로** 준다. 동물병원 10,617건을 그렇게 받았다.
  → "최초 1회 전체분 → 이후 증분" 2단 구조는 짓지 않는다.
- **인증키**: 공공데이터포털은 계정당 1개(`DATA_GO_KR_KEY`)이고 데이터셋별로 활용신청을 건다.
  **인허가 데이터도 이제 여기 하나로 통일됐다** — 별도 사이트·별도 키는 없다.
  반드시 **Decoding 키**를 쓴다(§ETL 주의).
...
CREATE TABLE regions (
  ...
  localdata_cd    TEXT,                  -- 개방자치단체코드 (인허가 데이터의 지역코드)
...
- 핀 50개 초과 시 클러스터링.
- 선택 지역에서 지도를 멀리 이동하면 **"이 지역에서 다시 검색"** 버튼 노출.
  · ⚠️ `regions` 에는 읍면동 **중심좌표만** 있고 경계 다각형이 없다 — "경계 밖"은 물을 수 없다.
    **고른 지역 중심에서 3km** 넘게 멀어지면 뜨는 것으로 정한다 (D-86).
  · 누르면 지도 한가운데에서 **가장 가까운 읍면동으로 갈아탄다.** 좌표 반경으로 조회하지 않는다 —
    앱의 모든 화면이 읍면동 코드 위에 서 있다 (D-24·D-54·D-59).
...
- **하단에 출처와 기준일 필수 표기** — 예: `출처: 지방행정인허가데이터 · 기준일 2026-08-28`
...
>
> ⚠️ **지도 바탕은 예외다.** 카카오맵에는 밤 스타일이 없다(`setMapStyle` 은 서버에 등록된 것만
> 받고 공개된 것은 `default` 하나) — `DimScreenLayer` 로 **바탕만** 덮어 톤을 맞춘다 (D-87).
