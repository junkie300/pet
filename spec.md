...
- **LOCALDATA 좌표**: WGS84가 아닌 TM 계열(EPSG:5174 추정) → 변환 필요.
  ⚠️ **1단계 착수 시 실제 응답으로 좌표계를 반드시 검증할 것.**
...
- **트래픽 한도**: 공공데이터포털 기본 1,000건/일. 활용사례 등록 시 증량 신청 가능.
  ETL은 **증분(변경분)만** 수집하도록 설계.
...
> 기준 데이터는 **TourAPI가 제공하는 법정동코드정보** 를 출발점으로 사용한다.
...
포함: S-01 지역 선택 / S-02 지도 / S-03 상세 / S-07 즐겨찾기 / S-08 출처
제외: 입양 탭, 식당, 관광, 로그인, 여행 묶음, 응급 모드
