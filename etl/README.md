...
- 0단계에 남은 것은 없다. `localdata_cd` 까지 다 찼다 — 빈 곳은 `전남광주통합특별시` 한 행뿐이고
  그건 코드가 둘로 갈려 **비우기로 한 자리**다 (D-48).
- ⚠️ **LOCALDATA(localdata.go.kr) 는 2026-04-16 에 닫혔다** (D-93). 인허가 데이터는 공공데이터포털로
  통합됐고, 우리가 부르는 API 는 처음부터 그쪽이었다 — 2단계 미용업도 `15154944` 로 같은 키를 쓴다.
  개방자치단체코드 엑셀은 `docs/` 에 커밋돼 있다. **받아 둔 사본이 이제 원본이다.**
